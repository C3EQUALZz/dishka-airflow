"""APP scope lifecycle for the listener with a sync container."""
...
async def test_app_dependency(
    ...
    async with create_airflow_env(
        ...
        @inject
        def task(app_dep: FromDishka[AppDep], mock: FromDishka[Mock]) -> str:
            mock(app_dep)
            return "ok"
...
async def test_app_scope_reused_across_runs(
    ...
    async with create_airflow_env(
        ...
        @inject
        def task(app_mock: FromDishka[AppMock]) -> None:
