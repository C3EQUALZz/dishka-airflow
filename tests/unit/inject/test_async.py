"""inject decorator behaviour with an async container."""
...
@pytest.mark.asyncio()
async def test_inject_without_dependencies_is_passthrough() -> None:
    @inject
    async def task(value: int) -> int:
        return value + 1

    assert await task(41) == 42
...
async def test_inject_keeps_regular_arguments(
    ...
    async with create_airflow_env(
        ...
        @inject
        async def task(value: int, request_dep: FromDishka[RequestDep]) -> str:
            return f"{value}-{request_dep}"

        assert await task(42) == f"42-{REQUEST_DEP_VALUE}"
